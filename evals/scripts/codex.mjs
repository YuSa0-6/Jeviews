...
const KNOWN = new Set([
  'input_unchecked_use',
  'input_missing_unhandled',
  'error_empty_catch',
  'error_success_after_failure',
  'error_unhandled_promise',
  'secret_hardcoded',
  'secret_logged',
  'format_indentation',
  'format_quotes',
...
function main() {
  ...
  const files = new Set(
    readFileSync(join(evalDir(name), `${subset}.files.txt`), 'utf8')
      .trim()
      .split('\n'),
  );
  ...
  console.log(
    `${name}/${subset}: ${n} findings in ${Object.keys(out).length} files (${skipped} lines skipped) -> ${target}`,
  );
}
