...
const files = readFileSync(join(evalDir(name), `${subset}.files.txt`), 'utf8')
  .trim()
  .split('\n');
...
const markAll = (check, value, source, only = files) => {
  for (const f of only) if (!truth[f][check]) set(f, check, value, source);
};
...
if (r.language === 'typescript') {
  ...
  if (/TS5112/.test(tsc.out) || (tsc.code !== 0 && !/TS\d{4}/.test(tsc.out)))
    throw new Error('tsc did not run: ' + (tsc.err || tsc.out).slice(0, 300));
  tools.tsc = { code: tsc.code, diagnostics: (tsc.out.match(/error TS61(33|92|98)/g) ?? []).length };
  markAll('lint_unused_import', 'clean', 'tsc', tsFiles);
  markAll('lint_unused_variable', 'clean', 'tsc', tsFiles);
  markAll('lint_unused_param', 'clean', 'tsc', tsFiles);
  for (const m of tsc.out.matchAll(/^(.+?)\((\d+),(\d+)\): error TS(6133|6192|6198)(.*)$/gm)) {
    ...
    if (code === '6192' || /^\s*import\b/.test(src)) check = 'lint_unused_import';
      ...
      !/^\s*(const|let|var)\b/.test(src) &&
      new RegExp(`[(,]\\s*(\\.\\.\\.)?${ident.replace(/[$]/g, '\\$')}\\b`).test(src)
    )
      check = 'lint_unused_param';
  ...
  for (const c of [
    ...
    const src = join(repoDir(name), c);
    if (existsSync(src)) copyFileSync(src, join(wd, c));
  }
...
if (r.language === 'ruby') {
  const COPS = {
    'Lint/UnusedMethodArgument': 'lint_unused_param',
    'Lint/UnusedBlockArgument': 'lint_unused_param',
    'Lint/UselessAssignment': 'lint_unused_variable',
    'Lint/SuppressedException': 'error_empty_catch',
    'Lint/LiteralAsCondition': 'lint_constant_condition',
    'Lint/UnreachableCode': 'lint_unreachable',
    'Layout/IndentationWidth': 'format_indentation',
    'Layout/IndentationConsistency': 'format_indentation',
    'Layout/IndentationStyle': 'format_indentation',
    'Layout/SpaceAroundOperators': 'format_spacing',
    'Layout/SpaceAfterComma': 'format_spacing',
    'Layout/SpaceInsideBlockBraces': 'format_spacing',
    'Layout/SpaceAroundKeyword': 'format_spacing',
    'Layout/SpaceInsideParens': 'format_spacing',
  ...
  tools.rubocop = {
    code: rb.code,
    inspected: j.files.length,
    offenses: j.files.reduce((n, f) => n + f.offenses.length, 0),
  };
  if (j.files.length !== files.length)
    throw new Error(`rubocop inspected ${j.files.length}/${files.length}: ${rb.err.slice(0, 300)}`);
  const broken = new Set(
    j.files.filter((f) => f.offenses.some((o) => o.cop_name === 'Lint/Syntax')).map((f) => f.path),
  );
  tools.rubocop.syntaxErrors = broken.size;
  for (const f of j.files) {
    if (broken.has(f.path)) continue;
    for (const o of f.offenses) {
      const check = COPS[o.cop_name];
      if (!check) continue;
      if (check === 'format_quotes') {
        const src = readFileSync(join(wd, f.path), 'utf8');
        if (!(/'[^'\n]*'/.test(src) && /"[^"\n]*"/.test(src))) continue;
      }
      set(f.path, check, 'problem', 'rubocop', `${o.cop_name} L${o.location.line}`);
      seen.add(f.path + check);
    }
  }
  for (const c of [...new Set(Object.values(COPS))].filter((c) => c !== 'error_empty_catch'))
    markAll(
      c,
      'clean',
      'rubocop',
      files.filter((f) => !broken.has(f)),
    );
}

if (r.language === 'python') {
  const RULES = {
    F401: 'lint_unused_import',
    F841: 'lint_unused_variable',
    ARG001: 'lint_unused_param',
    ARG002: 'lint_unused_param',
    ARG004: 'lint_unused_param',
    S110: 'error_empty_catch',
    E101: 'format_indentation',
    W191: 'format_indentation',
  };
  ...
  for (const d of diags) {
    const check = RULES[d.code];
    if (!check) continue;
    set(d.filename, check, 'problem', 'ruff', `${d.code} L${d.location.row}`);
  }
  for (const c of [...new Set(Object.values(RULES))].filter((c) => c !== 'error_empty_catch'))
    markAll(c, 'clean', 'ruff');
...
if (r.language === 'go') {
  ...
  if (build.code === 0) {
    markAll('lint_unused_import', 'clean', 'go-compiler', goFiles);
    markAll('lint_unused_variable', 'clean', 'go-compiler', goFiles);
  } else console.error('go vet failed in clone; skipping compiler negatives:', build.err.slice(0, 300));
  const sc = run(
    'mise',
    ['exec', 'go@latest', '--', 'go', 'run', 'honnef.co/go/tools/cmd/staticcheck@latest', '-f', 'json', './...'],
    { cwd: repoDir(name) },
  );
  ...
  for (const line of sc.out.split('\n').filter(Boolean)) {
    ...
    if (d.code === 'SA4006' || d.code === 'SA4009')
      set(rel, 'lint_unused_variable', 'problem', 'staticcheck', `${d.code} L${d.location.line}`);
    if (d.code === 'SA9003') set(rel, 'error_empty_catch', 'problem', 'staticcheck', `${d.code} L${d.location.line}`);
    if (d.code === 'SA4004')
      set(rel, 'lint_constant_condition', 'problem', 'staticcheck', `${d.code} L${d.location.line}`);
  }
}

const codex = readJson(join(evalDir(name), `${subset}.codex.json`), { findings: {} }).findings;
for (const [f, checks] of Object.entries(codex))
  for (const [c, v] of Object.entries(checks)) if (truth[f] && !truth[f][c]) truth[f][c] = v;

const human = readJson(join(evalDir(name), `${subset}.human.json`), {});
for (const [f, checks] of Object.entries(human))
  for (const [c, v] of Object.entries(checks)) if (truth[f]) truth[f][c] = { ...v, source: v.source ?? 'human' };

const out = { repo: name, subset, pin: r.pin ?? null, tools, files: truth };
writeJson(join(evalDir(name), `${subset}.expected.json`), out);
const counts = {};
for (const checks of Object.values(truth))
  for (const [c, v] of Object.entries(checks)) {
    counts[c] ??= { problem: 0, clean: 0 };
    counts[c][v.truth]++;
  }
console.log(`${name}/${subset}: ${files.length} files  tools=${JSON.stringify(tools)}`);
for (const [c, v] of Object.entries(counts).sort())
  console.log(`  ${c.padEnd(26)} problem=${String(v.problem).padStart(3)} clean=${String(v.clean).padStart(3)}`);
