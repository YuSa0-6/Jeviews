...
for (const [name, r] of Object.entries(config.repos)) {
  ...
  const ranked = files
    .map((f) => [hash32(name + ':' + f), f])
    .sort((a, b) => a[0] - b[0])
    .map((x) => x[1]);
  const tests = ranked.filter(isTest),
    code = ranked.filter((f) => !isTest(f));
  let ti = 0,
    ci = 0;
  for (const [subset, size] of Object.entries(subsets)) {
    ...
    for (const f of pick) {
      mkdirSync(join(wd, dirname(f)), { recursive: true });
      cpSync(join(dir, f), join(wd, f));
    }
    git(wd, ['init', '-q']);
    git(wd, ['add', '-A']);
    git(wd, [
      '-c',
      'user.email=eval@jeviews',
      '-c',
      'user.name=eval',
      'commit',
      '-q',
