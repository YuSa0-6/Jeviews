...
import { evalDir, git, JEVIEWS, run, workDir } from './lib.mjs';
...
const res = run('node', [join(JEVIEWS, 'dist/cli.js'), 'all', '--concurrency', concurrency], {
  cwd: wd,
  env: process.env,
});
...
writeFileSync(
  file,
  JSON.stringify({
    meta: { repo: name, subset, questionVersion: qv, jeviewsCommit: commit, run: prev + 1 },
    output: out,
  }) + '\n',
);
const c = {};
for (const f of out.files) c[f.verdict ?? 'null'] = (c[f.verdict ?? 'null'] ?? 0) + 1;
console.log(
  `${name}/${subset} ${qv} r${prev + 1}: ${JSON.stringify(c)} status=${out.run.status} usd=${out.run.usage.costUsd} -> ${file.replace(JEVIEWS + '/', '')}`,
);
